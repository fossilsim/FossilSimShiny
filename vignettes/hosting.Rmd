---
title: "Hosting FossilShinyApp on a Web Server (Debian 11)"
author: "Titouan Chambe"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Hosting FossilShinyApp on a Web Server (Debian 11)}
...
* [Dependencies]
  * [R]
  * [Dependencies for building shiny server from source]
  * [R packages we need]
...
Some dependencies used by FossilSim require a rather recent version of R. At the time of making this, on debian bullseye, you need a newer version of R than the one supplied by the advanced packaging tool (apt).\
\
...
\
If you are using a different distro, you can use https://cran.rstudio.com/bin/linux/ to find the right PPA.\
\
...
* **Python 3.6+**, preinstalled on Debian 11.
...
I recommend creating a new directory: `/srv/shiny-server/FossilShinyApp/`.

Then, you can grab the `app.R` file, the `/www/` folder and the `/R/` folder from <a href = "https://github.com/fossilsim/shiny/tree/master/inst/shinyApp">here</a> and copy them to `/srv/shiny-server/FossilShinyApp/`.
...
Futhermore you can change the port used in `/etc/shiny-server/shiny-server.conf`.
...
You should then be able to access your app using any web browser and going to the following we address:

http://YOUR_IP:3838/FossilSimShiny/


## Further Reading

* <a href="https://docs.rstudio.com/shiny-server/">Administrative guide to shiny server</a>
